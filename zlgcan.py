    ...
    def __init__(self):
        ...
        if platform.system() == "Windows":
            if ctypes.sizeof(ctypes.c_voidp) == 4:
                driver_dir = os.path.join(os.path.dirname(__file__), 'dll', 'x86')
            elif ctypes.sizeof(ctypes.c_voidp) == 8:
                driver_dir = os.path.join(os.path.dirname(__file__), 'dll', 'x64')
            self.__dll = windll.LoadLibrary(os.path.join(driver_dir, "zlgcan.dll"))
        else:
